...
void* mm_malloc (size_t size) {
  ...
  if (blockSize >= reqSize + MIN_BLOCK_SIZE) {
    ...
    *((size_t *)POINTER_ADD(ptrFreeBlock, blockSize - WORD_SIZE)) = (blockSize - reqSize) | TAG_PRECEDING_USED;
    ...
    // in this case, ptrFreeBlock's size is reqSize
    ptrFreeBlock->sizeAndTags = (reqSize | TAG_USED) | TAG_PRECEDING_USED;
  } else {
    // set the next block's preceding tag used
    NextBlockSize = SIZE(*((size_t *)POINTER_ADD(ptrFreeBlock, blockSize)));
    *((size_t *)POINTER_ADD(ptrFreeBlock, blockSize)) = NextBlockSize | TAG_PRECEDING_USED | TAG_USED;
    if (NextBlockSize != 0) {
      NextBlock = (BlockInfo *) POINTER_ADD(ptrFreeBlock, blockSize);
      *((size_t *) POINTER_ADD(NextBlock, NextBlockSize - WORD_SIZE)) = NextBlockSize | TAG_PRECEDING_USED | TAG_USED;
    }
    // in this case, ptrFreeBlock's size is blockSize
...
void mm_free (void *ptr) {
  ...
  BlockSizeTag = blockInfo->sizeAndTags;

  // set tag not used
  blockInfo->sizeAndTags = BlockSizeTag ^ 1;
  *((size_t *)POINTER_ADD(blockInfo, SIZE(BlockSizeTag) - WORD_SIZE)) = BlockSizeTag ^ 1;

  // set next block's preceeding tag not used
  NextBlockSizeTag = *((size_t *)POINTER_ADD(blockInfo, SIZE(BlockSizeTag)));
  *((size_t *)POINTER_ADD(blockInfo, SIZE(BlockSizeTag))) = NextBlockSizeTag ^ TAG_PRECEDING_USED;
  if (SIZE(NextBlockSizeTag) != 0) {
      NextBlock = (BlockInfo *) POINTER_ADD(blockInfo, SIZE(BlockSizeTag));
      *((size_t *) POINTER_ADD(NextBlock, SIZE(NextBlockSizeTag) - WORD_SIZE)) = NextBlockSizeTag ^ TAG_PRECEDING_USED;
    }
